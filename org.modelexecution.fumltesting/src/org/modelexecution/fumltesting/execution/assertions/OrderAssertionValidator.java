...
public class OrderAssertionValidator {
	 ...
	 * @return
	 */
	public boolean checkOrder(String parentNodeName,
			List<NodeSpecification> specifiedOrder,
			List<ActivityNodeExecution> executedNodes) {
		this.executedNodes = executedNodes;
		boolean result = compare(getTopNodes(parentNodeName, executedNodes),
				specifiedOrder);
	 ...
	 * @return
	 */
	private boolean compare(List<ActivityNodeExecution> executedNodes,
			List<NodeSpecification> nodeOrderList) {
			...
			if (nodeOrderList.get(i).getNode() != null) {
				if (nodeOrderList.get(i).getSubOrder() != null) {
					...
					if (nodeOrderList.get(i).getNode() instanceof CallBehaviorAction) {
						activityName = ((CallBehaviorAction) nodeOrderList.get(
								i).getNode()).getBehavior().getName();
					}
					if (nodeOrderList.get(i).getNode() instanceof CallOperationAction) {
						activityName = ((CallOperationAction) nodeOrderList
								.get(i).getNode()).getOperation().getMethods()
								.get(0).getName();
					}
					List<ActivityNodeExecution> executedSubNodes = validator
							.getTopNodes(activityName, this.executedNodes);
					List<NodeSpecification> subNodesSpecification = nodeOrderList
							.get(i).getSubOrder().getNodes();
					boolean subOrderValid = validator.checkOrder(activityName,
							subNodesSpecification, executedSubNodes);
				...
				if (executedNodeIndex >= executedNodes.size()
						|| !nodeOrderList
								...
								.getNode()
								.getName()
								.equals(executedNodes.get(executedNodeIndex)
										.getNode().name)) {
					return false;
				}
				executedNodeIndex++;
				...
				if (nodeOrderList.get(i).getJoker().equals("*")) {
					if (i < nodeOrderList.size() - 1) {
						if (nodeOrderList.get(i + 1).getNode() == null) {
							System.out
									.println("Use of subsequent star joker not allowed!");
							System.out.println("Assertion skipped!");
							return false;
						}
						String nextNode = nodeOrderList.get(i + 1).getNode()
								.getName();
						if (nodeOrderList.get(i + 1).getNode() != null)
							executedNodeIndex = getExecutedNodeIndex(nextNode,
									executedNodes);
					}
				}
			}
		...
		if (executedNodeIndex < executedNodes.size() - 1
				&& (nodeOrderList.get(nodeOrderList.size() - 1).getJoker() == null || !nodeOrderList
						.get(nodeOrderList.size() - 1).getJoker().equals("*"))) {
			return false;
		}
		return true;
	}
	 ...
	 * @return
	 */
	private int getExecutedNodeIndex(String name,
			List<ActivityNodeExecution> executedNodes) {
	 ...
	 * @return
	 */
	private List<ActivityNodeExecution> getTopNodes(String activityName,
			List<ActivityNodeExecution> executedNodes) {
		...
		for (ActivityNodeExecution node : executedNodes) {
			if (((Activity) node.getNode().owner).name.equals(activityName)
					&& (node.getNode() instanceof Action
							|| node.getNode() instanceof InitialNode || node
