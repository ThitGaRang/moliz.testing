...
public class TestLangScopeProvider extends XbaseScopeProvider {
	...
	public IScope getScope(EObject context, EReference reference) {
		...
		if (context instanceof VarDeclaration
				&& reference.getName().equals("ref")) {
			Activity activity = ((TestCase) context.eContainer())
			...
			return Scopes.scopeFor(nodes, new UmlQualifiedNameProvider(),
					IScope.NULLSCOPE);
		}
		...
		if (context instanceof TestCase
			...
			for (ActivityNode node : activity.getNodes()) {
				if (node.getOwner().equals(activity)
						&& node instanceof ActivityParameterNode)
					nodes.add(node);
			}
			return Scopes.scopeFor(nodes, new UmlQualifiedNameProvider(),
					IScope.NULLSCOPE);
		...
		if (context instanceof ObjectSpecification
				&& reference.getName().equals("att")) {
			...
			return Scopes.scopeFor(properties, new UmlQualifiedNameProvider(),
					IScope.NULLSCOPE);
		}
		...
		if (context instanceof OrderAssertion
				&& reference.getName().equals("node")) {
			TestCase testCase = (TestCase) context.eContainer();
			Activity activity = testCase.getActivityUnderTest();
			ArrayList<ActivityNode> nodes = new ArrayList<ActivityNode>();
			for (ActivityNode node : activity.getNodes()) {
				if (node instanceof Action || node instanceof ActivityFinalNode
						|| node instanceof InitialNode)
					nodes.add(node);
			}
			return Scopes.scopeFor(nodes, new UmlQualifiedNameProvider(),
					IScope.NULLSCOPE);
		...
		if (context instanceof Link
				&& (reference.getName().equals("sourceProperty") || reference
						.getName().equals("targetProperty"))) {
			ArrayList<Property> properties = new ArrayList<Property>();
			for (Property property : ((Link) context).getAssoc()
					.getMemberEnds()) {
				properties.add(property);
			}
			return Scopes.scopeFor(properties, new UmlQualifiedNameProvider(),
		...
		if (context instanceof StateAssertion
				&& reference.getName().equals("referenceAction")) {
			Activity activity = ((TestCase) ((StateAssertion) context)
					.eContainer()).getActivityUnderTest();
			ArrayList<ActivityNode> nodes = new ArrayList<ActivityNode>();
			for (ActivityNode node : activity.getNodes()) {
				if (node instanceof Action)
					nodes.add(node);
		...
		if (context instanceof StateAssertion
				&& reference.getName().equals("untilAction")) {
			Activity activity = ((TestCase) ((StateAssertion) context)
					.eContainer()).getActivityUnderTest();
			ArrayList<ActivityNode> nodes = new ArrayList<ActivityNode>();
			for (ActivityNode node : activity.getNodes()) {
		...
		if (context instanceof PropertyStateExpression
				&& reference.getName().equals("property")) {
			Type type = ((PropertyStateExpression) context).getPin().getRef()
					.getType();
			ArrayList<Property> properties = new ArrayList<Property>();
			...
			for (Element element : package_.getOwnedElements()) {
				if (element instanceof Association
						&& ((Association) element).getEndTypes().contains(type)) {
					for (Property property : (((Association) element)
							.getNavigableOwnedEnds())) {
						if (!property.getType().getName()
								.equals(type.getName()))
							properties.add(property);
					}
				}
