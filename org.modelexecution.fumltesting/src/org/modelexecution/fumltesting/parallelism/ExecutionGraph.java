...
public class ExecutionGraph {	
	...
	public ExecutionGraphNode getRoot(){return root;}
	...
	/** Checks if successor can be added to the node based on logical dependency from the trace. */
	...
	/** Returns all logical successors of the node, and logical successors of the successors. */
	private List<ActivityNodeExecution> getAllLogicalSuccessors(ActivityNodeExecution nodeExecution, List<ActivityNodeExecution> allLogicalSuccessors){
		for(ActivityNodeExecution successor: nodeExecution.getLogicalSuccessor()){
			if(!allLogicalSuccessors.contains(successor))allLogicalSuccessors.add(successor);
			if(successor.getLogicalSuccessor().size()>0)getAllLogicalSuccessors(successor, allLogicalSuccessors);
		}
		return allLogicalSuccessors;
	}
	
	/** Returns all logical predecessors of the node, and logical predecessors of the predecessors. */
	private List<ActivityNodeExecution> getAllLogicalPredecessors(ActivityNodeExecution nodeExecution, List<ActivityNodeExecution> allLogicalPredecessors){
		for(ActivityNodeExecution predecessor: nodeExecution.getLogicalPredecessor()){
			if(!allLogicalPredecessors.contains(predecessor))allLogicalPredecessors.add(predecessor);
			if(predecessor.getLogicalPredecessor().size()>0)getAllLogicalPredecessors(predecessor, allLogicalPredecessors);
		}
